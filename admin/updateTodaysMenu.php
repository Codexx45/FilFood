...
<head>
	...
	<title>Update Today's Menus</title>
</head>
...
<body>
	<div id="wrapper">
		...
		<div id="page-content-wrapper">
			...
			<div class="container-fluid d-flex align-items-end pb-3 text-warning rounded-bottom shadow-lg " style="background-image: url('../img/bg2.jpeg'); height: 25vh; width: auto; background-repeat: no-repeat; background-position: center; background-size: cover;">
			...
			<section class="col bg-white">
				<form action="php-operations/updateMenuItems.php" method="post">
					<div class="d-flex flex-column">
						<div class="container-fluid py-5 px-4 order-2" style="max-width: 500px;">
							<input type="submit" class="btn btn-success container shadow-lg rounded-3 h-100 py-3 fw-bolder fs-3" value="UPDATE MENU">
						</div>

						<div class="container-fluid row m-auto mt-5">
							<?php
                $sql = "SELECT * FROM food INNER JOIN menu ON food.food_id=menu.food_id WHERE date(menudate)=date(now()) ORDER BY category, food_name";
                ...
                while($row = mysqli_fetch_assoc($query)){
                    ...
                    ?>
							<div class="col-6 col-sm-4 col-md-3 col-lg-2 py-2 overflow-hidden">
								<input type="checkbox" class="btn-check" id="food_<?php echo $food_id; ?>" name="food[]" value="<?php echo $food_id; ?>" autocomplete="off" checked />
							...
							<?php }
                $sql2 = "SELECT * FROM food WHERE food_id NOT IN (SELECT food.food_id FROM food INNER JOIN menu ON food.food_id=menu.food_id WHERE date(menudate)=date(now())) ORDER BY category, food_name";
